...
if __name__ == '__main__':
    ...
    trainingdata = X_train.reshape(len(X_train), X_train.shape[1])
    testdata = X_test.reshape(len(X_test), X_test.shape[1])
    r.fit(trainingdata)
    y_train = handling.y_train 
    y_test = handling.y_test

    underlying_feature = r.transform(trainingdata)
    underlying_feature_test = r.transform(testdata)

    # train with CNN classifier 
